...
class SelfPlayEnv(gym.Env):
    ...
    def __init__(self, server_cmd, opponent_models, learner_seat: int = 0, base_seed: int = 0):
        ...
        self.proc = subprocess.Popen(list(server_cmd), stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                                     text=True, bufsize=1)
        raw_opponents = list(opponent_models) if isinstance(opponent_models, (list, tuple)) else [opponent_models]
        ...
        # Resolve string/JSON specs once here. Self-play opponents are deliberately frozen, including
        # run specs in live mode, so a trainer cannot change an opponent mid-episode or mid-run.
        resolver = ControllerResolver()
        self.opp_pool = [
            resolver.resolve(opponent) if isinstance(opponent, (str, dict)) else opponent
    ...
    def _scripted(self):
        ...
        return isinstance(self.opp, str) or (
            isinstance(self.opp, ResolvedController) and self.opp.model is None
        )
    ...
    def _is_scripted_opponent(opponent):
        return isinstance(opponent, str) or (
            isinstance(opponent, ResolvedController) and opponent.model is None
        )
    ...
    def _play_opponent(self, v):
        ...
        while not v["terminated"] and not v["truncated"] and int(v["seat"]) == self.opp_seat:
            ...
            if isinstance(self.opp, ResolvedController):
                assert self.opp.model is not None and self.opp.algorithm is not None
                validate_inference_input(self.opp, observation, mask)
                a = predict_resolved(self.opp.model, self.opp.algorithm, observation, mask)
            else:
                a = predict(self.opp, observation, mask)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self._scripted():
            if isinstance(self.opp, ResolvedController):
                msg[f"p{self.opp_seat}"] = self.opp.server_controller
            else:
