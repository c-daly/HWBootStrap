...
| `MATCH_TERMINAL_RECONNECT_SECONDS` | int | `600` | no | no | How long after a match finishes a player may still reconnect to it and be dealt the final state. Valid range 0..86400; `0` closes the window. The last `APPLY` of a game is the frame most likely to be lost, and a player whose socket dropped a moment earlier has no other way to learn how the game ended. Only `completed` is reachable this way - `expired` and `abandoned` never are. |
