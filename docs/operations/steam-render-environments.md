...
| `MATCH_MAX_OPEN_MATCHES_PER_IP` | int | `3` | no | no | Matches one address may successfully allocate inside a rolling ten-minute window; over the cap `POST /api/v1/steam/matches` answers **429** `rate_limited`. Valid range 1..1000. The per-minute rate limiter bounds how often a caller may ask; this bounds how much durable state a patient one leaves behind, so it counts the matches actually created and never charges a create that found the lobby already had a match. Counted in memory and lost on restart: no IP address reaches the database. |
...
| `MATCH_RETENTION_ACTIVE_IDLE_DAYS` | int | `7` | no | no | Silence after which a started match is set to `abandoned` and its live sockets are closed with **1001**. Valid range 1..365. A game in progress is never disturbed however long it has been running, as long as players keep playing it. |
