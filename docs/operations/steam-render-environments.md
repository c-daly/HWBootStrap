...
| `MATCH_MAX_RECHECKS_PER_CADENCE` | int | `256` | no | no | Sockets one heartbeat will re-check the credential of, oldest-checked first, drained eight at a time until the batch is done or the next heartbeat is due. Valid range 8..4096. It has to be large enough that the whole socket budget fits inside one `MATCH_CREDENTIAL_RECHECK_SECONDS` interval, or the documented cadence is a fiction and the last socket on a busy host is asked about minutes late. Exactly one pass runs at a time: a tick that finds the previous one still going cancels it and starts none of its own, and a single check that has not answered within five seconds is abandoned so its worker can move on. Sockets a pass did not reach keep their place at the front of the queue. |
