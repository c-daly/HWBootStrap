...
| `MATCH_OUTBOUND_QUEUE_BYTES` | int | `4194304` | no | no | Bytes one `/ws/v2` connection may have queued before it is closed with 1008 `slow client`. Valid range 65536..67108864. A frame count is not a memory bound on its own: a `START` carrying a long journal is orders of magnitude bigger than an `APPLY`, so a queue well inside `MATCH_OUTBOUND_QUEUE_CAPACITY` can still be holding megabytes, once per socket. |
| `MATCH_MAX_SOCKETS_PER_IP` | int | `8` | no | no | Sockets one address may hold on `/ws/v2` at once, counted before the upgrade is accepted; over the cap the request is answered **429**. Valid range 1..256. A match needs two; the cap is there because an accepted socket costs a receive buffer, a writer task and a registry entry before it has proved anything. |
| `MATCH_TERMINAL_RECONNECT_SECONDS` | int | `600` | no | no | How long after a match finishes a player may still reconnect to it, or join it again over HTTP, and be dealt the final state. Valid range 0..86400; `0` closes the window. The last `APPLY` of a game is the frame most likely to be lost, and a player whose socket dropped a moment earlier has no other way to learn how the game ended. Any terminal status counts as long as the match actually started; one that ended without ever starting has no game to show anybody. |
| `MATCH_MAX_RECHECKS_PER_CADENCE` | int | `256` | no | no | Sockets one heartbeat will re-check the credential of, oldest-checked first, drained eight at a time until the batch is done or the next heartbeat is due. Valid range 8..4096. It has to be large enough that the whole socket budget fits inside one `MATCH_CREDENTIAL_RECHECK_SECONDS` interval, or the documented cadence is a fiction and the last socket on a busy host is asked about minutes late. Two bounds hold whatever the store does. Exactly one pass runs at a time - a tick that finds the previous one still going cancels it and starts none of its own - and at most **16 store calls are outstanding at any moment, ever**, counting the ones this host has given up waiting for. A check that has not answered within five seconds is abandoned so its worker can move on, but the call it left behind keeps its slot until it really finishes, so a store that ignores cancellation cannot accumulate orphaned queries. When all 16 slots are held, re-checks pause and every socket stays open; the host says so once a minute until the store answers again. Sockets a pass did not reach keep their place at the front of the queue. |
| `MATCH_CREDENTIAL_RECHECK_SECONDS` | int | `60` | no | no | How often a live `/ws/v2` socket has its credential re-checked against the store. Valid range 5..3600. A handshake is a moment and a match is an hour: a credential revoked by the same player reconnecting elsewhere would otherwise leave a socket that was legitimate when it opened holding a seat for as long as the client cared to. Expiry is checked every heartbeat regardless of this setting. |
| `ALLOWED_WEB_ORIGINS` | comma list of absolute URLs | empty | no | no | Browser origins this server serves. Two effects, both browser-only. A websocket upgrade carrying an `Origin` is refused unless that origin is on this list or equals the request `Host`; an `Origin` header that is present and unreadable (a sandboxed frame sends the literal `null`) is refused outright, while a request with no `Origin` at all passes, which is every native client. Separately, `GET /games` and the health endpoints carry a named CORS policy over exactly this list, GET only. `/api/v1/steam/*` and both websocket routes never emit CORS headers: a credential-bearing endpoint must not advertise itself as a cross-site target. Compared as scheme plus authority, case-insensitively, so a trailing slash or an explicit default port makes no difference. None of this protects a native client, whose only real proof is the join credential. |
...
| `MATCH_MAX_OPEN_MATCHES_PER_IP` | int | `3` | no | no | Matches one address may successfully allocate inside a **rolling** ten-minute window; over the cap `POST /api/v1/steam/matches` answers **429** `rate_limited`. Valid range 1..1000. Rolling rather than fixed: the count is taken over the trailing ten minutes, so waiting for a boundary and spending the whole budget again a second later does not work. Admission is a reservation taken before any network or database call, so concurrent creations from one address cannot all be told there is room for one more; a request that creates nothing hands its seat straight back, and one whose write outcome is unknown is charged. A retry for a lobby that already has a match is served even at the cap, because it writes nothing. Addresses are bucketed: an IPv4 address arriving over IPv6 (`::ffff:a.b.c.d`) counts as the IPv4 one, and an IPv6 client counts by its **/64 prefix**, which is the smallest unit an ISP hands out. At most 10,000 addresses are tracked; a saturated map refuses addresses it has not seen rather than evicting a live window, because evicting one would hand that caller its whole budget back. Counted in memory and lost on restart: no IP address reaches the database. |
| `MATCH_RETENTION_SWEEP_MINUTES` | int | `60` | no | no | How often the retention sweeper runs. Valid range 1..1440. Registered only when `DATABASE_URL` is set. Each pass applies the four statements in [match-data-retention.md](./match-data-retention.md) in one transaction and logs row counts only, never an identifier. |
| `MATCH_RETENTION_WAITING_MINUTES` | int | `30` | no | no | Age at which a match that never started is set to `expired`. Valid range 1..10080. |
| `MATCH_RETENTION_ACTIVE_IDLE_DAYS` | int | `7` | no | no | Silence after which a started match is set to `abandoned` and its live sockets are closed with **1001**. Valid range 1..365. A game in progress is never disturbed however long it has been running, as long as players keep playing it. Closing the sockets is bounded and retried rather than best-effort: a match whose gate is busy is left exactly as it was and tried again on the next sweep, and an eviction that throws or stalls is logged and stepped over rather than being allowed to stop the sweeper. |
| `MATCH_RETENTION_TERMINAL_DAYS` | int | `90` | no | no | How long a finished match is kept before the row is hard-deleted. Valid range 1..3650. Deleting the match takes its seats, its command journal and its join credentials with it by foreign-key cascade; nothing is ever reaped one seat at a time. |
| `MATCH_RETENTION_CREDENTIAL_HOURS` | int | `24` | no | no | How long past its expiry a join credential row is kept before it is deleted, whether or not it was revoked. Valid range 1..8760. |
...
The console provider is configured in `Program.cs` and **renders logging scopes**, which is not cosmetic:
match and lobby ids travel as scopes rather than being repeated in every message, so a formatter that
drops scopes drops exactly the identifiers an operator searches for during an incident.

| Environment | Formatter | Why |
| --- | --- | --- |
| Production | `AddJsonConsole`, `IncludeScopes = true`, `UseUtcTimestamp = true` | Render indexes structured lines, so `MatchId` and `LobbyId` become fields the log search can filter on. One timezone, and the one every other timestamp in this system uses. |
| Everything else | `AddSimpleConsole`, `IncludeScopes = true` | A person is reading it. |

The default provider is cleared first, so no line is written twice.
