namespace HexWars.NetServer.Runtime
{
        ...
        ILogger<RecoveryStartupService> logger) : IHostedService, IDisposable
    {
        ...
        public async Task StopAsync(CancellationToken cancellationToken)
        {
            ...
            {
                // Already disposed, so the loop is already cancelled and there is nothing left to stop.
                // Swallowed rather than propagated because the host stops hosted services in order and
                // abandons the rest on the first exception: a throw here would take the graceful shutdown
                // down with it, and the players whose sockets it was about to close with 1012 would get a
                // torn connection instead.
                return;
            }
