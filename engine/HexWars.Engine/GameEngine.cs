...
namespace HexWars.Engine
{
    ...
    public static class GameEngine
    {
        ...
        private static Result ApplyAttackUnit(GameState state, AttackUnit c)
        {
            ...
            var target = enemy.UnitsOnBoard[tUnitIdx];
            HexCoord targetCell = target.Cell;
            int targetElevation = target.Elevation;
            int targetDefense = target.Stats.Defense
                + state.Config.Terrain(state.Board.TileAt(target.Cell).Terrain).Defense;
            int buildCost = target.Stats.PointCost;
            ...
            var hurt = target.WithDamage(damage);
            bool killed = !hurt.IsAlive;
            var units = new List<Unit>(enemy.UnitsOnBoard);
            if (killed) units.RemoveAt(tUnitIdx); else units[tUnitIdx] = hurt;
            var newEnemy = new PlayerState(
                enemy.Id, enemy.Points, enemy.Barracks, units,
                enemy.Generators, enemy.DestroyedValue);
